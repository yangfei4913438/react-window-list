 ...
 */
export const createMeasureLayer = (debug: boolean) => {
  const container: HTMLDivElement = document.createElement('div', {});
  container.setAttribute('id', 'measure-layer');
  if (!debug) {
    container.style.cssText = Object.entries(containerStyle)
      .map(([k, v]) => `${k}:${v}`)
      .join(';');
  }
 ...
 */
const measureElement = (element: ReactElement, debug: boolean) => {
  const container = document.querySelector('#measure-layer') || createMeasureLayer(debug);
